...
class AFG3021B:
    ...
    def setAmplitude(self, amp):
        return self._generador.write(f'VOLT {amp}')
        
    def getAmplitude(self):
        return self._generador.query('VOLT?')
    
    def setCuadrada(self,canal):
        return self._generador.write(f'SOUR{canal}:FUNC:SHAP SQU')
    
    def setSin(self,canal):
        return self._generador.write(f'SOUR{canal}:FUNC:SHAP SIN')
    def setRamp(self,canal):
        return self._generador.write(f'SOUR{canal}:FUNC:SHAP RAMP')

    def setOffset(self,canal,value):
        return self._generador.write(f'SOUR{canal}:VOLT:LEV:INM:OFFS {value}mV')
...
class TDS1002B:
    ...
    def set_pk2pk(self,channel):        
#        self._osci.write("CH{0}:SCA {1}".format(channel, scale))
#        self._osci.write("CH{0}:POS {1}".format(channel, zero))
        self._osci.write("MEASUREMENT:IMMED:SOURCE CH{0}".format(channel))
        self._osci.write("MEASUREMENT:IMMED:TYPE pk2pk")
        return self._osci.query("MEASUrement:IMMed:TYPe?")
    def get_pk2pk(self,channel):
        self.set_pk2pk((self,channel))
        return  float(self._osci.query("MEASUrement:IMMed:VALue?")[:-1])
    ...
    def get_range(self, channel):
        ...
        return rango    
class GPIB0: 
    """Clase para el manejo de el multimetro de banco Hp34301 a traves de el 
    conector GPBI de NI usando PyVISA de interfaz
    """
    
    def __init__(self, name='GPIB0::23::INSTR'):
        self._multi = visa.ResourceManager().open_resource(name)
        self._multi.query("*IDN?")
        
    def get_voltage(self):
        return float(self._multi.query('MEASURE:VOLTAGE:DC?'))