...
maxim = 0.0048000000000000004  # alturas_osc[index_peaks_restric].max()
...
# Pantalla = np.load(r'1000 pantallas 1 us 10mv .npz')['screens']  # 1us
# Pantalla1 = np.load(r'1000 pantallas 1 us 10mv (2).npz')['screens']
Pantalla = np.concatenate((Pantalla, Pantalla1))

# %% CONFIGURACION DE ESCALAS DEMASES

data = Pantalla
escala = 0.5  # microsegundos, setear al cargar nuevamente los datos
resolucion = (escala/250)*1e-6  # Consutla
tiempo = np.arange(0, resolucion*(len(data)), resolucion)
...
ax[0].plot(np.array(alturas[0:len(t_a)])[1250:2500],
           np.array(t_a[0:len(t_a)])[1250:2500], "-", linewidth=0.8)
...
ax[0].set_ylabel('Tiempo [$\mu s$]', fontsize=16)
ax[0].grid(True)
ax[0].legend()
ax[0].set_title('Histograma de picos', fontsize=20)
...
el histograma de el threshold + el error instrumental y - el error instrumental
y voy a tomar como error la diferencia entre ambos'''
threshold = -0.0029354999999999997
res_volt = 10e-3/250  # en volts
t = np.array(t_a[0:len(t_a)])
...
fraccion = 10  # 10 es un buen numero

Tt = 2001
umbral1 = 0.0029354999999999997
distancia = 5
...
plt.clf()
plt.rc('font', family='serif', size=13)
bins = np.arange(-0.5, max(N1)+0.5, 1)
fig, ax = plt.subplots(1, 1, num=1, sharex=True)
# 1  BINS PARA EL HISTOGRAMA DE 1 US de acuerdo a la cantidad maxima de N1
bose_data = ax.hist(N1, bins, color='#0504aa', alpha=0.7,
                    rwidth=0.85, label='Mediciones', density=False)
...
# Ordenamos y salvamos la figura.
plt.tight_layout()
# plt.subplots_adjust(bottom=0.154, left=0.072, right=0.996, top=0.908)# escaleo para 1us
...
ax.set_ylabel("Ocurrencias", fontsize=18)
ax.ticklabel_format(axis='y', style='sci', scilimits=(1, 3))
ax.set_title('Ajuste Bose-Einstein', fontsize=20)
ax.set_xlim(-1, max(N1)+0.5)
ax.set_yscale('log')
ax.legend()
ax.grid(True)
...
plt.show()
plt.savefig('Ajuste BE log 0.5 us.png')
...
ax.set_title("Ajuste Poisson", fontsize=20)
ax.ticklabel_format(axis='Y', style='sci', scilimits=(1, 4))
...
plt.subplots_adjust(bottom=0.144, left=0.114, right=0.996, top=0.921)
plt.show()
plt.savefig('Ajuste poisson log 0.5 us.png')
...
# ax.set_ylabel("Ocurrencias", fontsize=18)
# ax.ticklabel_format(axis='y', style='sci', scilimits=(1, 3))
# ax.set_title('Histograma de picos con $T = 1 ~\mu s$', fontsize=20)
# ax.set_xlim(-1, max(N1)+0.5)
...
# bins = np.arange(-0.5, 40.5, 1)
# fig, ax = plt.subplots(1, 1, num=1, sharex=True)
# ax.errorbar(x, y, yerr=np.sqrt(y), fmt='+', label='Datos $T = 1~\mu s$')
# ax.plot(x, BE(x, *parametros_optimos), "o", label='Ajuste')
# ax.set_xlabel("Número de fotocuentas", fontsize=14)
# ax.set_ylabel("Ocurrencias", fontsize=14)
# ax.ticklabel_format(axis='y', style='sci', scilimits=(1, 3))
# ax.set_title('Ajuste Bose-Einstein', fontsize=16)
# ax.set_xlim(-1, 6)
# # ax.set_yscale('log')
# ax.legend()
# ax.grid(True)
...
# plt.tight_layout()
# plt.show()
# plt.savefig('Ajuste BE 1 us.png')
...
# plt.subplots_adjust(bottom=0.114, left=0.124, right=0.996, top=0.936)
# plt.show()
# plt.savefig('Ajuste poisson 250 ns.png')
# # plt.errorbar(x, y, yerr=np.sqrt(y), fmt='+', label='Datos')
# # plt.plot(x, poiss1(x, *parametros_optimos), ".", label='Ajuste')
# # plt.yscale('log')
# # plt.legend()
...
# ax.set_title("Ajuste Combinacion", fontsize=18)
# ax.ticklabel_format(axis='Y', style='sci', scilimits=(1, 4))
